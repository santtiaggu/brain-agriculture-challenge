...
async def post_producer(
    payload: ProducerInput, current_user: dict = Depends(get_current_user)
):
    return await create_producer(payload)


@router.post("/producers/list", response_model=ProducerListResponse)
async def post_list_producers(
    pagination: PaginationInput, current_user: dict = Depends(get_current_user)
):
...
async def get_producer_by_id(
    producer_id: int, current_user: dict = Depends(get_current_user)
):
...
async def put_producer(
    producer_id: int,
    payload: ProducerInput,
...
async def delete_producer_route(
    producer_id: int, current_user: dict = Depends(get_current_user)
):
